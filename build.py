      ...
      - name: Commit dated snapshot
        run: |
          ...
          if ! git diff --cached --quiet; then
            ...
            echo "Failed to publish refreshed snapshot after 3 attempts" >&2
            exit 1
          fi
...
def build(config,data,output=None,now=None):
    ...
    page('/sources/',f'Sources and verification method | {brand}','Official source coverage, collection failures and the verification method behind VPSDealRadar.',source_content)
